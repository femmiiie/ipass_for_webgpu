...
wgpu::BindGroupLayoutEntry RenderPass::CreateTextureLayout(uint16_t binding, wgpu::ShaderStage visibility)
{
...
wgpu::BindGroupLayoutEntry RenderPass::CreateSamplerLayout(uint16_t binding, wgpu::ShaderStage visibility, wgpu::SamplerBindingType type)
{
  wgpu::BindGroupLayoutEntry entry;
  entry.binding = binding;
  entry.visibility = visibility;
...
wgpu::BindGroupLayoutEntry RenderPass::CreateBufferLayout(uint16_t binding, wgpu::ShaderStage visibility, wgpu::BufferBindingType type, uint64_t minBindingSize)
{
  wgpu::BindGroupLayoutEntry entry;
  entry.binding = binding;
  entry.visibility = visibility;
...
wgpu::BindGroupEntry RenderPass::CreateBinding(uint16_t entry, wgpu::Buffer& buffer)
{
  wgpu::BindGroupEntry binding = wgpu::Default;
  binding.binding = entry;
...
wgpu::BindGroup RenderPass::CreateBindGroup(std::vector<wgpu::BindGroupEntry> bindings, wgpu::BindGroupLayout& bindLayout)
{
  wgpu::BindGroupDescriptor desc;
...
void RenderPass::ClearBuffer(wgpu::CommandEncoder& encoder, wgpu::Buffer& buffer)
{
